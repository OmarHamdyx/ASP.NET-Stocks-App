...
namespace StocksApp.Filters
{
	public class OrderFilter : IAsyncActionFilter, IOrderedFilter
	{
		...
		public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
		{
			...
			{
				StockDetailsViewModel orderRequest = context.ActionArguments.Values.FirstOrDefault(arg => arg is StockDetailsViewModel) as StockDetailsViewModel;
				if (orderRequest != null)
				{
					...
					if (!context.ModelState.IsValid)
					{
						...
						context.Result = new RedirectToActionResult("GetStockDetails", null, new { Errors = errors,Order = orderRequest.Quantity });
						return;
					}
				}
			}
